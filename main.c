...
int parse_common_args(int argc, char*argv[])
{
        ...
        for (i = 1; i < argc; ++i) {
                if (argv[i][0] == '-') {
                        switch (argv[i][1]) {
                        case 'c': strncpy(config_name, argv[i] + 2, sizeof(config_name) - 1); break;
                        case 'l': def_debug = strtol(argv[i] + 2, NULL, 0); break;
                        case 'd': if (argv[i][2]) is_daemon = strtol(argv[i] + 2, NULL, 0); else is_daemon = 1; break;
                        case 'g': if (argv[i][2]) guarded = strtol(argv[i] + 2, NULL, 0); else guarded = 1; break;
...
void sighandler(int sig)
{
        ...
        if (sig == SIGTERM && ss.stop) signal(SIGTERM, SIG_DFL);
        ss.stop = 1;
}
